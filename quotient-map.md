...
It's surprising how little time or effort is dedicated to discussing what the process of *abstraction* is. By abstration, we mean omitting details from some process, so that the transformation  becomes simpler, but remains faithful to the original process. 

We discuss how _quotient maps_ are a useful formalism for expressing abstractions, how to attempt to identify or enumerate abstractions for a given problem domain and what can they be used for. We arrive at a lookout from which the possibility of identifying unexpected abstractions emerges, which is exciting. 
...
Consider a mapping from a finite set to itself, defined using some mathematical language. We explicitly constrain  X to be finite, in order to avoid an overly broad discussion. A mathematical language ultimately specifies a time-based transformation, so our transformation can be expressed in whichever form one likes:
...
Even though a typical diagram of a discrete function from a math textbook appears innocous, `f` can be of arbitrary complexity. Solving problems such as inverting a function are out of reach in general, for complex choices of `f`.

### Abstraction comes to rescue

Given the mapping `f`, consider a problem such as inverting `f` for a given output value. Alternatively, consider the problem of finding a collision for `f`, two values that map to the same output. Finally, the problem of computing `f(x)` from `x` itself may be hard.

Suppose that `f` is such that the problems mentioned above are not necessarily easy. For example, `f` maybe a recurrence relation used inside a cryptographic hash function such as SHA-2, may be a difficult Diophantine equation modulo a large number.
...
For each $x \xrightarrow{\text{q}} y$, we have $[x] \xrightarrow{\text{Q}}[y]$. 

In other words, if $x$ is mapped to $y$ by the original map, the corresponding "blurred" version of $x$ and $y$ must be in correspondence with the quotient map $Q$. 
...
<img src="other-pics/quotient/comm.png" alt="First Image" width="50%">
