...
<p align="center"> <img src="other-pics/quotient/quotient.png" alt="First Image" width="55%"> </p> 
...
In other words, if `x` is mapped to `y` by the original map, the corresponding "blurred" version of `x`
 and `y` must be in correspondence with each other via Q via the quotient map. 
...
As for the bit-wise differences from the other two papers, they are natural as, apart from modular addition, all other hash found operations are bit-wise. It also makes sense from human perception viewpoint, where one would write an X where a bit is "activated", that is, where a difference is injected. The multi-bit differences are a natural extension of single-bit differences.

This all leads to the following intuition: **it is very likely a generic procedure exists, from which the three abstractions discussed in the previous section can be derived**. We put forward a research project, some of which questions were (possibly) discussed in literature, perhaps in another form:

* Determine the systematic procedure used to derive abstractions such as the three above. If such a derivation already exists, connect it to the hash function papers and explain the transformation. Otherwise, derive the transformation yourself and deduce the abstractions from the papers. 
* Specifically to hash function differential trail search heuristics: which abstraction is _optimal_ in the sense that it is not too heavy-weight on memory/cache/CPU and yet derives solutions? There's a clear tradeoff here, as the memory tables holding abstracted language propagation rules grow exponentially. This can be tested in practice with hash functions. From the existing literature, it is not clear how to determine the "optimal" language for searching for collisions. 
* Given an existing abstraction that works, is there a generic procedure or a closure that make it more or less coarse; how can abstractions be tuned along a fixed axis? Do different abstractions live across axes, or are independent?
* What mappings are at the ends of the spectrum in the number of abstractions they amend to, vs. those that have a minimal number of abstractions. Could mappings without any meaningful abstractions correspond to something like to S-boxes in cryptography? Non-linearity?
* Study a problem of your choice (such as, a difficult Diophantine equation) and introduce a novel abstraction which sheds new light on the problem. 
